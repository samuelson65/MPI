...
class RETAINNumpy:
    ...
    def forward(self, idx_A, idx_B):
        ...
        score_A = float(np.dot(hid_A, self.W_alpha.squeeze()) + self.b_alpha[0])
        score_B = float(np.dot(hid_B, self.W_alpha.squeeze()) + self.b_alpha[0])
        ...
        # Step 5: Output
        logit = float(np.dot(context, self.W_out.squeeze()) + self.b_out[0])

...
COLUMN FORMAT REQUIRED IN BOTH CSVs:
    ...
    claimA_provider_tax_id      provider tax ID on index claim
    claimA_drg_code             DRG code on index claim
    claimA_diag_codes           pipe-delimited ICD-10 codes  e.g. I50.43|E11.65|I10
    claimA_proc_codes           pipe-delimited procedure codes  e.g. 99232|93010
    ...
    claimB_provider_tax_id      provider tax ID on readmission claim
    claimB_drg_code             DRG code on readmission claim
    claimB_diag_codes           pipe-delimited ICD-10 codes
    claimB_proc_codes           pipe-delimited procedure codes
...
def build_samples(df, has_label=True):
    ...
    for _, row in df.iterrows():
        vA = visit_tokens(row, "claimA_")
        vB = visit_tokens(row, "claimB_")
        db = gap_days(row)
        vB += [f"DAYS_{min(db,30)}"] if db >= 0 else ["DAYS_UNK"]
        ...
        s = {
            ...
            "proc_A":          parse_codes(row.get("claimA_proc_codes","")),
            "proc_B":          parse_codes(row.get("claimB_proc_codes","")),
...
def score_and_explain(model, samples, vocab):
    ...
    for s in samples:
        ...
        rows.append({
            ...
            "claimA_all_diagnoses":           "|".join(s["diag_A"]),
            "claimA_procedures":              "|".join(s["proc_A"]),
            "claimA_drg":                     s["drg_A"],
            ...
            "claimB_all_diagnoses":           "|".join(s["diag_B"]),
            "claimB_procedures":              "|".join(s["proc_B"]),
            "claimB_drg":                     s["drg_B"],
